...
- [ ] 認可コードフローでPKCEなしのリクエストを送信できる
- [ ] サーバーがcode_verifier不在を検出し、攻撃成功を表示する
- [ ] 正しいPKCE実装を行うと「ボス撃破」判定になる
- [ ] 攻撃の解説と防御方法がゲーム内で表示される

### Boss 2: State Mismatch (CSRF)
- [ ] state パラメータなしの認可リクエストを再現できる
- [ ] CSRF攻撃シナリオが視覚的に表現される
- [ ] 正しいstate検証を実装すると「ボス撃破」判定になる
- [ ] 攻撃フローの図解が表示される

### Boss 3: Nonce Replay Attack
- [ ] nonce再利用によるトークンリプレイ攻撃を再現できる
- [ ] ID Tokenのnonce検証の重要性が体験できる
- [ ] 正しいnonce検証を実装すると「ボス撃破」判定になる

### Boss 4: JWKS Rotation Failure
- [ ] JWKSエンドポイントのキーローテーション失敗を再現できる
- [ ] 古い鍵で署名されたトークンの検証失敗が体験できる
- [ ] 適切なJWKSキャッシュ戦略を実装すると「ボス撃破」判定になる

### Boss 5: Logout Hell
- [ ] フロントチャネルログアウトの不完全実装を再現できる
- [ ] セッション残存によるセキュリティリスクが体験できる
- [ ] 正しいRP-Initiated Logout + Back-Channel Logoutを実装すると「ボス撃破」判定になる

### 共通要件
- [ ] 各ボスに挑戦履歴がブラウザに保存される（localStorage）
- [ ] ゲーム画面にボス一覧（進捗付き）が表示される
- [ ] Docker Compose一発で開発環境が起動する
- [ ] 各ボスの解説ページに RFC/仕様書へのリンクがある
